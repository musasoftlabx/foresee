...
if ($_GET['context'] === 'display') {
    ...
    $countQuery = "SELECT COUNT(id) FROM scans_failed";
    ...
    $scans = ChainPDO("SELECT id, code, barcode, scannedBy, serialNumber, batteryLevel, DATE_FORMAT(scannedOn, '%a, %D %b %Y at %r') AS scannedOn FROM scans_failed $filter $sort $order $limit")->fetchAll();
    ...
    $dataset = ChainPDO("SELECT SUBSTR(code, 1, 5) AS code, section, SpecialCode, color, size, name, scans_failed.barcode, COUNT(code) AS quantity
        FROM scans_failed
        LEFT JOIN products
        ON scans_failed.barcode = products.barcode
        WHERE storeId = ?
        GROUP BY scans_failed.barcode, code
