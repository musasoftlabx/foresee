          ...
          <v-row no-gutters>
            <v-col cols="10">
              ...
              <div style="color: #bdffff" class="text-caption mt-0 ml-1">
                All scanned locations will be picked from this store.
              </div>
            </v-col>
        ...
        >
          <v-form ref="CreateLocation" v-model="valid">
            <v-row no-gutters justify="center">
              ...
              <v-col cols="12" class="px-2">
                <v-text-field
                  v-model="scanCharsLimit"
                  :rules="[validators.required]"
                  label="Scan Characters Limit *"
              ...
              <v-col cols="12" class="px-2">
                <div class="text-caption mt-0 ml-1 mb-n2">
                  Set operand (Enforces barcode limits)
                </div>
                <v-radio-group v-model="strict">
                  <v-radio
                    :label="`Barcode is strictly equals to ${scanCharsLimit} chars.`"
                    :value="true"
                  />
                  <v-radio label="Barcode varies" :value="false" />
                </v-radio-group>
              </v-col>
              ...
              <v-col cols="12" align="end" class="px-2 pb-6">
                <v-btn
                  ...
                  text
                  :loading="loading"
                  :disabled="!valid || loading || strict === null"
                  @click="SetWorkingStore"
    ...
    SetWorkingStore() {
      ...
      this.promiseFetch(this.$store.getters.fetchTimeout)(
        fetch(`${this.$store.getters.endpoint}Stores/`, {
          ...
          body: JSON.stringify({
            workingStore: JSON.stringify(this.store),
            workingDate: this.date.scanDate,
            workingStoreDate:
              this.store.code +
              "-" +
              Number(this.date.scanDate.split("-").join("")),
            scanCharsLimit: this.scanCharsLimit,
            strict: this.strict ? 1 : 0,
          }),
        })
      )
        .then((response) =>
          response.json().then((res) => {
            if (response.status > 201) {
              ...
              this.visible = false;
              this.$emit("NewWorkingStore", this.store);
            }
          })
        )
        .catch(() => {
