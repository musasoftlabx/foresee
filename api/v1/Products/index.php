...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_GET['Upload'])) {
        ...
        if (in_array($ext, $whitelist)) {
            ...
            $dataset = [];
            $columns = ChainPDO("SHOW COLUMNS FROM products")->fetchAll(PDO::FETCH_COLUMN);
            $accepted = ['section', 'barcode', 'name', 'color', 'size', 'length', 'SpecialCode'];
    ...
    if (isset($_GET['Commit'])) {
        ...
        try {
            ...
            ChainPDO("TRUNCATE TABLE products");

            $fields = '';

            foreach ($_JSON['mappings'] as $key => $value) {
                $fields .= $value['column'] . ','; 
            }
            ...
            foreach ($sheetData as $key => $value) {
                if ($key < 100000) {
                    if (count($value) > 0) {
                        ...
                        /* foreach ($_JSON['mappings'] as $k => $v) {
                            $index = array_search($v['excelField'], $excelFields);
                            
                            if ($k == $index) {
                                $result = str_replace("'", ",", trim($value[$index]));
                            } else {
                                $result = '';
                            }
                        ...
                        foreach ($_JSON['mappings'] as $k => $v) {
                            $set .= "'" . str_replace("'", ",", trim($value[array_search($v['excelField'], $excelFields)])) . "',";
                        }

                        $set = rtrim($set, ',');

                        ChainPDO("INSERT INTO products ($fields) VALUES ($set)");
                    }
                }
            }
            ...
            echo json_encode([
                "count" => ChainPDO("SELECT COUNT(PK) FROM products")->fetchColumn()
            ]);
        } catch(\PhpOffice\PhpSpreadsheet\Reader\Exception $e) {
            echo json_encode([
